...
public class FileUtils {
    public static void createPluginFileInDataFolder(String fileName){
        ...
        if (!file.exists()){
            InteractiveSigns.getInstance().saveResource(fileName, false);
    ...
    public static void createPluginFileInFolder(String fileName, String folderName, boolean isReplace){
        ...
        if (!file.exists()){
            InteractiveSigns.getInstance().saveResource(fileName, isReplace);
            Text.sendMessageToConsole("<green>File \"" + fileName + "\" was created!");
        }
    }
    ...
    private static File createDataFolder(){
        File dataFolder = InteractiveSigns.getInstance().getDataFolder();
        if (dataFolder.mkdir())
    ...
    public static FileConfiguration getYamlConfiguration(String fileName){
        return YamlConfiguration.loadConfiguration(new File(InteractiveSigns.getInstance().getDataFolder(), fileName));
    }
}

...
public class WorldEditListener {
    ...
    private static class SignExtent extends AbstractDelegateExtent {
        ...
        public <T extends BlockStateHolder<T>> boolean setBlock(BlockVector3 pos, T block) throws WorldEditException {
            Timer.wait(0, () -> removeItemDisplayFromSign(BukkitAdapter.adapt(world, pos).add(0.5,0.5,0.5)));
            return super.setBlock(pos, block);
        }
        public <T extends BlockStateHolder<T>> boolean setBlock(int x, int y, int z, T block) throws WorldEditException {
            Timer.wait(0, () -> removeItemDisplayFromSign(new Location(world, x + 0.5, y + 0.5, z + 0.5)));
        ...
        public <B extends BlockStateHolder<B>> int setBlocks(final Region region, final B block) throws MaxChangedBlocksException {
            Timer.wait(0, () -> region.forEach(pos -> removeItemDisplayFromSign(BukkitAdapter.adapt(world, pos).add(0.5,0.5,0.5))));
            return super.setBlocks(region, block);
        }
        public int setBlocks(final Region region, final Pattern pattern) throws MaxChangedBlocksException {
            Timer.wait(0, () -> removeItemDisplayFromSign(region));
        ...
        public int setBlocks(final Set<BlockVector3> vset, final Pattern pattern) {
            Timer.wait(10, () -> vset.forEach(pos -> removeItemDisplayFromSign(BukkitAdapter.adapt(world, pos).add(0.5,0.5,0.5))));

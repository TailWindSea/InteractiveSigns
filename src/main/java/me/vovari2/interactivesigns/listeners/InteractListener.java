        ...
        switch(event.getAction()) {
            case RIGHT_CLICK_BLOCK: {
                ...
                if (!WorldGuardUtils.canInteractWithSign(blockLocation, player)){
                    Delay.run(() -> player.sendMessage(Text.get("you_cant_use_that_here")), player, "cant_use_this_here", 20);
                    event.setCancelled(true);
                    return;
                }
                ...
                if (isOccupiedByText(signBlock.getSide(side).lines())){
                    if (isDye(item.getType()))
                        return;
                    Delay.run(() -> player.sendMessage(Text.get("you_cant_place_item_because_sign_has_text")), player, "cant_use_this_here", 20);
                    event.setCancelled(true);
                    return;
                }
